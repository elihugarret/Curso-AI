...
local field = field2D.new(2115, 496)
...
function reset()
	field:set(function(x, y)
		if y == past then
			return math.random(3) - math.random(1)
		else
			return 0
		end
	end)
...
function update()
	...
	for x = 0, field.width do		 
		 ...
		 local E = field:get(x / math.random(9), past*3)
		 local W = field:get(x + math.random(2), past/3)
		 C = transition_rule_30(C, E, W)
		 field:set(C, x, future)                
	end	
	past = future	
	future = (past - math.random(192)) % field.height
      
end
function draw()
  --red green blue
  draw2D.color(0,0.9,0.1)
  field:draw()
end

...
local numboids = 120
local viewrange = 0.23
local closerange = viewrange * 0.05
local copyfactor = 20
local centerfactor = 10.2
local avoidfactor = 11.9
local smooth = 0.34
local speed = 0.004
...
function update()
  ii = audioin.ExecuteFFT(1,32)
  if ii[10] > 300 then print(ii[10]); iii = ii[10] end
	...
	for i, self in ipairs(boids) do
		...
		if num_near > 0 then
			...
			self.direction = self.direction + 0.4 * srandom()
...
function draw()	
	...
	if showneighbors then
		for i, self in ipairs(boids) do
			for j, p in ipairs(self.relatives) do
				draw2D.color(1,.6, iii/1000,0.47)
				draw2D.line(self.pos.x*(iii/250), self.pos.x^p.y, self.pos.x*(iii/100), self.pos.x + p.y)	
			end
		end	
	end
	...
	for i, self in ipairs(boids) do
		...
		if showinfluences and #self.relatives > 0 then
			-- avoid
			draw2D.color(0, 0,.3)
			draw2D.line(0, 0, self.avoid.y, self.avoid.y)			
			
			-- copy
			draw2D.color(0.3, 0.2, 0)
			...
			-- total influence
			draw2D.color(0, 1, 1)
			draw2D.line(0, 0, self.influence.y, self.influence.x)	
		end
		
		draw2D.rotate(self.direction)
		-- draw view:
		if showview then
			draw2D.color(0, 1, .75,0.05)
			draw2D.arc(0, 0, -math.pi/9, math.pi/9, viewrange)
		end
		
		draw2D.scale(0.001)
		-- draw body:
		draw2D.color(1, 0, 1,0.2)
		draw2D.line(1, 0, -3, 0)
		draw2D.rect(1.5, 0, 1)
